...
class SmapGANABLl1Model(BaseModel):
    """
    ...
    def l1_origin_plus_rgb_grad_plus_gradlossP(self, fake, real):
        return 10.*torch.nn.L1Loss()(fake, real) + L1_rgb_grad_loss(fake, real) + grad_loss(fake, real) + grad_loss(fake.transpose(-1,-2), real.transpose(-1,-2))
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionCycleBP= torch.nn.L1Loss()
...
#         self.loss_G_B = self.criterionGAN(self.netD_B(torch.cat((self.real_B, self.fake_A), 1)), True)
        ...
        if lambda_paired_loss > 0:
            self.loss_paired_AB = self.criterionCycleBP(self.fake_B, self.real_B) * lambda_B * lambda_paired_loss * 10.
